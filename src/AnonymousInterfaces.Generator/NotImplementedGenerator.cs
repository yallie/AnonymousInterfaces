...
/// </summary>
[Generator]
public class NotImplementedGenerator : IIncrementalGenerator
{
    ...
    private static ClassInfo GetClassInfo(GeneratorSyntaxContext context)
    {
        ...
        // Check for [NotImplemented] or [NotImplementedAttribute]
        ...
        foreach (var interfaceSymbol in allInterfaces)
        {
            foreach (var member in interfaceSymbol.GetMembers())
            {
                ...
                if (impl == null)
                {
                    switch (member)
                    {
                        // Only ordinary methods – skip property/event accessors
    ...
    {
        // Diagnostics: how many classes were found
    ...
    private static string GenerateSource(ClassInfo info)
    {
        ...
        // Generate missing members (with 4‑space indentation)
    ...
    {
        string interfaceName = method.ContainingType.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
        ...
        var paramStrings = method.Parameters.Select(p =>
        {
            ...
            string name = p.Name;
            // Do not add default values to avoid CS1066 warning
    ...
    {
        string interfaceName = prop.ContainingType.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
    ...
    {
        string interfaceName = ev.ContainingType.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
